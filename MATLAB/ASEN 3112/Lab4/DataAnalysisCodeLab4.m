...
L_sq = 11 + 3/8;
A_sq = 0.25*0.0625*2+(0.25-0.0625*2)*0.0625*2;
Yield_sq = 0.3671;
y_sq = 0.125;
Pcr_sq = 232.78;
...
figure
hold on
% plot([0:1/16:2],lb_sq1,'o')
displacement = linspace(0,2);
force = Pcr_sq*(1+pi^2/(8*L_sq^2)*displacement.^2);
plot([0,0,displacement],[0,Pcr_sq,force],'b')
plot([Yield_sq,Yield_sq],[0,250])
...
legend('Predicted', 'Plastic Post-Buckling', 'Raw Data', 'Fitted Line')
ylim([0 250])
grid on
...
figure
hold on
% plot([0:1/16:2],lb_bar1,'o')
displacement = linspace(0,2);
force = Pcr_bar*(1+pi^2/(8*L_sq^2)*displacement.^2);
plot([0,0,displacement],[0,Pcr_bar,force],'b')
plot([Yield_bar,Yield_bar],[0,150])
